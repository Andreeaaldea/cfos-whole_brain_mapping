...
```{r read alcohol data}
dane<-fread("pe_cerebellum_cortex.csv")
...
# Save function (only defined once)
save_pheatmap_png <- function(x, filename_prefix, filename_suffix, width=1800, height=800, res = 300) {
    filename = paste(filename_prefix, filename_suffix, ".png", sep = "")
    png(filename, width = width, height = height, res = res)
    grid::grid.newpage()
...
# Loop through groups and save data
for (grupa in group_labels) {
  # Check if data exists
  if (!is.null(lista.danych[[grupa]]) && !is.null(correlations.within.categories[[grupa]])) {
    
    # Save original data
    original_data <- lista.danych[[grupa]]
    write.csv(original_data, file = paste0(saving_dir, "/", grupa, "_original_data.csv"), row.names = TRUE)
    
    # Save correlation matrix
    correlation_matrix <- correlations.within.categories[[grupa]]
    write.csv(correlation_matrix, file = paste0(saving_dir, "/", grupa, "_correlation_matrix.csv"), row.names = TRUE)
    
    # Create and print heatmap
    my_heatmap <- pheatmap(correlation_matrix, 
                           ...
                           fontsize = 12, fontsize_col = 4, fontsize_row = 4,
                           main = grupa)

    print(my_heatmap)
    
    # Save the heatmap if save flag is TRUE
    if (save) {
      save_pheatmap_png(my_heatmap, paste(saving_dir, grupa, sep = ""), "-anatomical-order", width = 28000, height = 26000, res = 3000)
    }
  } else {
    warning(paste("Data for group", grupa, "is missing or not valid"))
  }
}
```
...
generate_distances_heatmap<-function(grupa, distances) {
  my_heatmap<-pheatmap(as.matrix(distances[[grupa]]),
                       ...
                       cellheight= 1.5,
                       cellwidth = 1.5,
                       show_rownames = T, fontsize_row = 3.5, 
                       show_colnames = T, fontsize_col = 3.5,
                       treeheight_row = 0,
                       treeheight_col = 90,
                       fontsize=8,
                       main = grupa)
...
if(save) {
  lapply(group_labels, function(x) {save_pheatmap_png(mapy[[x]], paste(saving_dir, x, sep=""), "-clustered", width=5600, height=5200, res = 300)})
}
```
